...
class UserServiceTest {
    ...
    void setUp() {
         ...
         sender = mock(KafkaProducer.class);
         userService = new UserService(userRepository, userMapper, sender);
    ...
    void whenCreateUserWithValidData() {
        ...
        when(sender.sendMessage("создан.", dto.getEmail())).thenReturn(true);
        ...
        verify(sender).sendMessage("создан.", entity.getEmail());
    ...
    void whenUpdateUserById() {
        ...
        when(userRepository.findById(validId)).thenReturn(Optional.ofNullable(entity));
        when(userRepository.save(entity)).thenReturn(entity);
        when(sender.sendMessage("обновлен.", entity.getEmail())).thenReturn(true);
        ...
        verify(userRepository).findById(validId);
        verify(userRepository).save(entity);
        verify(sender).sendMessage("обновлен.", entity.getEmail());
    ...
    void whenDeletedUserById() {
        //given
        when(userRepository.findById(validId)).thenReturn(Optional.ofNullable(entity));
        when(sender.sendMessage("удален.", entity.getEmail())).thenReturn(true);
        ...
        verify(sender).sendMessage("удален.", entity.getEmail());
    }
